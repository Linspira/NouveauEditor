...
Ext.define('Ext.data.reader.Reader', {
     ...
     * The name of a property within a raw row data object which indicates the class of record to be produced when reading the row.
     *
     * The type of model created defaults to the {@link Ext.data.Store#model model configured on the Store}, but stores may contain heterogeneous
     * model types, so this gives the capability to read different entity types into one store. This capability is likely to
     * be most useful in {@link Ext.tree.Panel TreePanel}s when levels of the tree may represent different entities.
     *
     * The value of the field in the row object is expected to be resolvable to a {@link Ext.data.Model Model} subclass.
     *
     * By default, the desired model will be assumed to be in the same namespace as this Reader's model as inherited
     * from the store it is serving:
     *
     *     xtype: 'tree',
     *     store: {
     *         id: 'offices',
     ...
     *             type: 'ajax',
     *             reader: {
     *                 typeProperty: 'ntype' // Which incoming property yields the child node class.
     *                                       // If needed, Will automatically prepend 'myApp.' from parent node's class.
     *             }
     *         }
     *     }
     *
     * This may consume a sequence data objects which looks like this:
     *
     *     [{
     *         name: 'North America'
     *     }, {
     ...
     *     }]
     *
     * The first row would create a `myApp.Territory` instance because that is the default type of the TreeStore.
     *
     * The second row would create a `myApp.State` instance because it uses the same model namespace as the TreeStore's model.
     *
     * The third row would create a `myApp.City` instance because it uses the same model namespace as the TreeStore's model.
     *
     * If more flexibility is needed, the typeProperty may be configured as an object of the form:
     *
     *     // The value of the incoming ntype property prepended by `'myOtherapp.'` to determine the model class name.
     *     {
     *         name: 'ntype',
     ...
     * or
     *
     *     // The value of the incoming ntype property is used as a key into the map to determine the model class name.
     *     {
     *         name: 'ntype',
     *         map: {
     *             State: 'someOtherApp.StateModel',
     *             City: 'anotherNamespace.City'
     *         }
     *     }
     *
     * The above example data would populate a heterogeneous linear store.
     *
     * For a {@link Ext.data.TreeStore TreeStore}, the data would look like:
     *
     *     [{
     *         name: 'North America',
     *         children: [{
     ...
     *     }]
     *
     * It may also be a function which returns either the class name, or the actual class to use:
     *
     *     // A function which determines the class name (or the class) from the raw node data
     *     function(rawData) {
     *         return 'myApp.namespace.' + rawData.metadata.nodeType;
     *     }
     *
     * The function is called with the scope (`this` reference) of this Reader.
     *
     * **Do not forget to `require` the extra classes when defining your store with the default model type.
     * Unless they are mentioned in a `requires` declaration, the classes will not be built into your application.**
     */
    ...
    readRecords: function(data) {
        ...
        //<debug>
        if (!me.model && !me.typeProperty) {
            Ext.Error.raise('A Reader must be configured with either a model, or a typeProperty so it can produce models from raw data');
        }
        //</debug>
     ...
     * @param {String} [defaultModelName] The name of the model class to read if there is
     * a need to override this Reader's configured {@link Ext.data.Store#cfg-model} class.
     * Used by TreeStore when the parent node's {@link Ext.data.TreeModel#childType} is passed in.
     * @return {Array} An array of records containing the extracted data
     * @private
     */
    extractData : function(root, defaultModelName) {
        var me = this,
            ModelClass,
            length  = root.length,
        ...
        for (i = 0; i < length; i++) {
            ...
            if (node.isModel) {
                ...
                // This Reader is configured to produce different model types based on a differentiator field in the incoming data
                if (me.typeProperty) {

                    // Interrogate the raw data to see what type it is
                    ModelClass = me.getModelClass(node);

                    // Use the data extraction function from the model class
                    dataConverter = ModelClass.getProxy().getReader().convertRecordData;
                }

                // This Reader is configured with a model.
                // 
                else {
                    // The default model name gets passed in by TreeStore using the childType of the parent node being loaded.
                    if (defaultModelName) {
                        ModelClass = me.resolveModelClass(defaultModelName, me.model);
                        ...
                        ModelClass = me.model;
                        dataConverter = me.convertRecordData;
                    }
                }
                ...
                dataConverter(convertedValues, node, record);
    ...
    /**
     * @protected
     * Given a row data object, decide the {@link Ext.data.Model model} class we are going to create.
     *
     * This defaults to the model we were configured with by the owning Store, but may be overridden if
     * the {@link #typeProperty} has been specified.
     *
     * The typeProperty may be
     *
     *    "City"
     *
     * or
     ...
     * or
     *
     *     // A function which determines the class name (or the class) from the raw data
     *     function(rawData) {
     *         return 'myApp.namespace.' + rawData.metadata.nodeType;
     *     }
     *
     * This method may be overridden in configuration of a Reader if the scheme outlined above is not sufficient to yield
     * a model class from the available data.
     *
     * @param {Object} data The raw row data object from which to determine the class to produce.
     */
    getModelClass: function(data) {
        ...
        if (typeProperty) {
            ...
            // typeProperty is just a simple property name to extract from data object.
            if (typeof typeProperty === 'string') {
                result = me.resolveModelClass(data[typeProperty], me.model);
            }
            // typeProperty is a function which retrieves the class name
            else if (Ext.isFunction(typeProperty)) {
                result = me.resolveModelClass(typeProperty.call(me, data), me.model);
            }

            // typeProperty is an object which describes how to extract a class name
            else {
                // Extract short form name
                modelName = data[typeProperty.name];
                ...
                // }
                if (typeProperty.namespace) {
                    modelName = typeProperty.namespace + '.' + modelName;
                }
                ...
                //     name: 'class',
                //     map: {
                //         City: 'myApp.namespace.City'
                //     }
                // }
                else {
                    modelName = typeProperty.map[modelName];
                }
                result = Ext.ClassManager.get(modelName);
            }
        }
        //<debug>
        if (!result) {
            Ext.Error.raise('Model class could not be ascertained for type: "' + modelName + '". Please check proxy->reader configuration');
        }
        //</debug>
        return result;
    },
    ...
    //
    // private
    // Resolves a passed class name using the namespace of the model passed in the second parameter.
    // If the modelName is already a model class, it is returned unchanged.
    resolveModelClass: function(modelName, model) {
        var defaultClassName,
            lastDot;

        // Not a fully namespaced class name. Needs resolving
        // TODO: In Sencha 5, ask the Schema what Model this name represents
        if (typeof modelName === 'string' && modelName.indexOf('.') === -1) {
            //<debug>
            if (!model) {
                Ext.Error.raise('If string form typeProperty is used on Reader, the store needs a model to provide the model class namespace');
            }
            //</debug>

            // Use the same namespace as the store's model
            defaultClassName = Ext.ClassManager.getName(model);

            lastDot = defaultClassName.lastIndexOf('.');
            if (lastDot !== -1) {

                // Prepend the "myApp.namespace." prefix of our default model name to the extracted model name
                modelName = defaultClassName.substring(0, lastDot + 1) + modelName;
            }
        }
