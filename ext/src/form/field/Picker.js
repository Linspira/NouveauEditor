...
Ext.define('Ext.form.field.Picker', {
    ...
    collapseIf: function(e) {
        ...
        if (!me.isDestroyed && !e.within(me.bodyEl, false, true) && !me.isElWithinPicker(e.target)) {
            me.collapse();
        }
    },
    ...
    onOtherFocus: function(dom) {
        if (this.hasFocus && !this.isElWithinPicker(dom)) {
            this.callParent([dom]);
        }
    },
    ...
    mimicBlur: function(e) {
        ...
        if (!picker || !me.isElWithinPicker(e.target)) {
    ...
    /**
     * returns true if the passed element is within the picker or its loadmask, or
     * is the picker or loadmask element
     * @private
     * @param {Ext.Element/HtmlElement} el
     * @return {Boolean}
     */
    isElWithinPicker: function(el) {
        var picker = this.picker,
            withinPicker = false,
            withinLoadMask = false,
            loadMask;

        if (picker) {
            loadMask = picker.loadMask,
            withinPicker = picker.el.contains(el),
            withinLoadMask = loadMask && loadMask.el ?
                    (loadMask.maskEl.contains(el) || loadMask.el.contains(el)) : false;
        }

        return withinPicker || withinLoadMask;
    }
