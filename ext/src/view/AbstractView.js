...
Ext.define('Ext.view.AbstractView', {
    ...
    // override.
    // mask Views using a LoadMask, not Element masking.
    mask: function(msg, msgClass) {
        var loadMaskConfig = {};

        if (msg) {
            loadMaskConfig.msg = msg;
        }
        if (msgClass) {
            loadMaskConfig.msgClass = msgClass;
        }
        this.setLoading(loadMaskConfig);
    },
    ...
    doFirstRefresh: function(store) {
        ...
        // Flag to prevent multiple "first" refresh from subsequent beforeLayout calls
